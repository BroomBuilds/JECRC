 ...
 * they are written to survive being quoted on their own: complete sentences, no
 * back-references.
 */
      ...
      <div className="grid gap-14 lg:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)] lg:gap-24">
        <SectionHeading
          ...
          title={
            <>
              The things people{" "}
              <span className="u-serif-italic text-crimson">actually ask</span>
            </>
          }
        ...
        <div className="border-t border-rule">
          {FAQS.map((f, i) => (
            ...
            >
              <summary className="flex cursor-pointer list-none items-start justify-between gap-6 py-6 text-[17px] font-bold leading-snug tracking-[-0.015em] text-ink transition-colors duration-300 hover:text-crimson md:text-[19px] [&::-webkit-details-marker]:hidden">
                {f.q}
                <Plus className="mt-1 h-4 w-4 shrink-0 text-crimson transition-transform duration-400 ease-out-expo group-open:rotate-45" />
              </summary>
              <p className="max-w-[64ch] pb-7 text-[15.5px] leading-[1.75] text-graphite">{f.a}</p>
            </details>
          ))}
        </div>
      </div>
